...
$(document).ready(function() {
    ...
    function videofunction(newId){
        for( i=0; i<videoPlaySectionData.length; i++){
            if("card"+videoPlaySectionData[i].id == newId)//card4
            {
            $("#video-player").attr("src", "https://player.vimeo.com/video/"+videoPlaySectionData[i].vimeoId);
            $("#views-count").html(videoPlaySectionData[i].views);
            $("#video-title").html(videoPlaySectionData[i].title);
    ...
    for(i=0; i<playlistData.length; i++){
        var result_card = createPlaylistCard(playlistData[i]);
        $("#playlist-wrapper").append(result_card);
